		...
		private function getFunctions( object:*, target:String = "" ):String
		{
			...
			try
			{
				...
				for ( i = 0; i < methodsArr.length; i++ )
				{
					...
					// Create the node
					xml += createNode( "node", { icon: ICON_FUNCTION, label: childName + "(" + argsString + "):" + returnType, args: argsString, name: childName, type: TYPE_FUNCTION, access: ACCESS_METHOD, returnType: returnType, target: childTarget });
					...
					{
						// Create the node
						xml += createNode( "parameter", { type: parseType( parameters[ n ].@type ), index: parameters[ n ].@index, optional: parameters[ n ].@optional }, true );
					}
			...
			if ( maxDepth == -1 || currentDepth <= maxDepth )
			{
				...
				try
				{
					...
					if ( functions && base == TYPE_FUNCTION )
					{
						...
						xml += createNode( "node", { icon: ICON_FUNCTION, label: "(Function)", name: "", type: TYPE_FUNCTION, value: "", target: target, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE }, true );
					}
					...
					else if ( type == TYPE_ARRAY || type == TYPE_VECTOR )
					{
						...
						if ( currentDepth == 1 )
							xml += createNode( "node", { icon: ICON_ROOT, label: "(" + type + ")", target: target });

						// Create the length property
						// The only property of the array
						xml += createNode( "node", { icon: ICON_VARIABLE, label: "length" + " (" + TYPE_UINT + ") = " + object[ "length" ], name: "length", type: TYPE_UINT, value: object[ "length" ], target: target + "." + "length", access: ACCESS_VARIABLE, permission: PERMISSION_READONLY }, true );
						...
						for ( i = 0; i < keys.length; i++ )
						{
							...
							if ( childType == TYPE_STRING || childType == TYPE_BOOLEAN || childType == TYPE_NUMBER || childType == TYPE_INT || childType == TYPE_UINT || childType == TYPE_FUNCTION )
							{
								...
								try
								{
									if ( !isXML )
									{
										xml += createNode( "node", { icon: ICON_VARIABLE, label: "[" + keys[ i ] + "] (" + childType + ") = " + printObject( object[ keys[ i ]], childType ), name: "[" + keys[ i ] + "]", type: childType, value: printObject( object[ keys[ i ]], childType ), target: childTarget, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE }, true );
									}
									else
									{
										xml += createNode( "node", { icon: ICON_VARIABLE, label: "[" + keys[ i ] + "] (" + childType + ")", name: "[" + keys[ i ] + "]", type: childType, value: "", target: childTarget, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE }, false );
							...
							{
								xml += createNode( "node", { icon: ICON_VARIABLE, label: "[" + keys[ i ] + "] (" + childType + ")", name: "[" + keys[ i ] + "]", type: childType, value: "", target: childTarget, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE });
						...
						for ( i = 0; i < properties.length; i++ )
						{
							...
							if ( childType == TYPE_STRING || childType == TYPE_BOOLEAN || childType == TYPE_NUMBER || childType == TYPE_INT || childType == TYPE_UINT || childType == TYPE_FUNCTION )
							{
								...
								try
								{
									if ( !isXML )
									{
										xml += createNode( "node", { icon: ICON_VARIABLE, label: properties[ i ] + " (" + childType + ") = " + printObject( object[ properties[ i ]], childType ), name: properties[ i ], type: childType, value: printObject( object[ properties[ i ]], childType ), target: childTarget, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE }, true );
									}
									else
									{
										xml += createNode( "node", { icon: ICON_VARIABLE, label: properties[ i ] + " (" + childType + ")", name: properties[ i ], type: childType, value: "", target: childTarget, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE }, false );
										xml += parseXML( isXMLString, childTarget + "." + "cildren()", currentDepth, maxDepth );
										xml += createNode( "/node" );
									}
								}
							...
							{
								xml += createNode( "node", { icon: ICON_VARIABLE, label: properties[ i ] + " (" + childType + ")", name: properties[ i ], type: childType, value: "", target: childTarget, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE });
					...
					else if ( type == TYPE_STRING || type == TYPE_BOOLEAN || type == TYPE_NUMBER || type == TYPE_INT || type == TYPE_UINT )
					{
						...
						try
						{
							if ( !isXML )
							{
								xml += createNode( "node", { icon: ICON_VARIABLE, label: "(" + type + ") = " + printObject( object, type ), name: "", type: type, value: printObject( object, type ), target: target, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE }, true );
							}
							else
							{
								xml += createNode( "node", { icon: ICON_VARIABLE, label: "(" + type + ")", name: "", type: type, value: "", target: target, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE }, false );
					...
					else
					{
							...
							if ( permission != PERMISSION_WRITEONLY )
							{
								...
								if ( childType == TYPE_STRING || childType == TYPE_BOOLEAN || childType == TYPE_NUMBER || childType == TYPE_INT || childType == TYPE_UINT || childType == TYPE_FUNCTION )
								{
									...
									try
									{
										if ( !isXML )
										{
											xml += createNode( "node", { icon: icon, label: childName + " (" + childType + ") = " + printObject( object[ childName ], childType ), name: childName, type: childType, value: printObject( object[ childName ], childType ), target: childTarget, access: variablesArr[ i ].access, permission: permission }, true );
										}
										else
										{
											xml += createNode( "node", { icon: icon, label: childName + " (" + childType + ")", name: childName, type: childType, value: "", target: childTarget, access: variablesArr[ i ].access, permission: permission }, false );
											xml += parseXML( isXMLString, childTarget + "." + "cildren()", currentDepth, maxDepth );
											xml += createNode( "/node" );
										}
									}
								...
								{
									xml += createNode( "node", { icon: icon, label: childName + " (" + childType + ")", name: childName, type: childType, target: childTarget, access: variablesArr[ i ].access, permission: permission });
						...
						if ( functions )
						{
							for ( i = 0; i < methodsArr.length; i++ )
							{
								...
								// Create the node
								xml += createNode( "node", { icon: ICON_FUNCTION, label: childName + "(" + args.join( ", " ) + "):" + returnType, args: args.join( ", " ), name: childName, type: TYPE_FUNCTION, access: variablesArr[ i ].access, returnType: returnType, target: childTarget }, true );
							}
						}
			...
			if ( maxDepth == -1 || currentDepth <= maxDepth )
			{
				...
				try
				{
					...
					for ( i = 0; i < childs.length; i++ )
					{
						...
						// Create the node
						xml += createNode( "node", { icon: childIcon, label: child.name + " (" + childType + ") " + childChildren, name: child.name, type: childType, value: printObject( child, childType ), target: childTarget, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE });
		...
		private function parseXML( node:*, target:String = "", currentDepth:int = 1, maxDepth:int = -1 ):String
		{
			...
			if ( maxDepth == -1 || currentDepth <= maxDepth )
			{
				...
				if ( target.indexOf( "@" ) != -1 )
				{
					...
					xml += createNode( "node", { icon: ICON_XMLATTRIBUTE, label: node, name: "", type: TYPE_XMLATTRIBUTE, value: node, target: target, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE }, true );
				}
				else if ( node.name() == null )
				{
					// Only a text value
					xml += createNode( "node", { icon: ICON_XMLVALUE, label: "(" + TYPE_XMLVALUE + ") = " + printObject( node, TYPE_XMLVALUE ), name: "", type: TYPE_XMLVALUE, value: printObject( node, TYPE_XMLVALUE ), target: target, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE }, true );
				}
				else if ( node.hasSimpleContent())
				{
					// Node with one text value and possible attributes
					xml += createNode( "node", { icon: ICON_XMLNODE, label: node.name() + " (" + TYPE_XMLNODE + ")", name: node.name(), type: TYPE_XMLNODE, value: "", target: target, access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE });
					...
					{
						xml += createNode( "node", { icon: ICON_XMLATTRIBUTE, label: "@" + node.attributes()[ i ].name() + " (" + TYPE_XMLATTRIBUTE + ") = " + node.attributes()[ i ], name: "", type: TYPE_XMLATTRIBUTE, value: node.attributes()[ i ], target: target + "." + "@" + node.attributes()[ i ].name(), access: ACCESS_VARIABLE, permission: PERMISSION_READWRITE }, true );
					}
